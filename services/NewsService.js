...
class NewsService {
  ...
  async friendComments(openid, page, limit) {
    let sql = 'SELECT u.name,c.*,b.`name` bookname,b.author,b.cover_url FROM `comment` c LEFT JOIN book_info b ON c.book_code=b.`code` LEFT JOIN users u ON c.writer_code=u.openid WHERE type=0  AND status="on" AND writer_code in  (SELECT user_code FROM fans WHERE fans_code = ?) LIMIT ?,?'
    let r = {}
