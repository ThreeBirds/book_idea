...
class NewsService {
  ...
  async hotComments() {
    let r = {}
    let sql = 'SELECT c.*,u.`name` username, u.headimgurl,b.`name` bookname FROM `comment` c INNER JOIN users u ON c.writer_code=u.openid  LEFT JOIN book_info b ON c.book_code=b.`code`  WHERE c.type=0  ORDER BY praise_count DESC LIMIT 50 '
