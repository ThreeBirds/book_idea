...
class MeService {
  ...
  async queryComment(openid, type, me, page, limit) {
    ...
    let sql0 = 'SELECT c.*,u.name,u.headimgurl,bi.`name` book_name,bi.cover_url,c3.content orgin_content FROM `comment` c INNER JOIN users u ON c.writer_code=u.openid LEFT JOIN book_info bi ON c.book_code=bi.`code` LEFT JOIN `comment` c3 ON c.reply_to=c3.id WHERE c.reply_to in ( ' +
